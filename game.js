// ======================================================
// DIAMOND CITY - v4.0
// Ayuntamiento (jefes) + Comisaría (defensa) + Guardado arreglado
// ======================================================
console.log('🚀 DIAMOND CITY v4.0');
...
const CONFIG = {
    ...
    API_BASE: '', // ← pon aquí la URL del Worker cuando esté listo
    PVP_ENABLED: false // ← activar solo cuando el Worker esté funcionando
};
...
const ECONOMIA = {
    ...
    // Ayuntamiento y Comisaría
    JEFE_COSTO_MULT: 150,        // costoBase = 150 × producción/h del edificio
    JEFE_SALARIO_PCT: 0.15,      // 15% de la producción va al jefe
    POLICIA_COSTO_BASE: 500,     // costo primera unidad
    POLICIA_CRECIMIENTO: 1.12,   // curva del costo
    POLICIA_EFECTIVIDAD_BASE: 1, // 1 policía = 1 tanque (punto de partida)
    JEFE_POLICIA_BONUS: 0.10     // +10% efectividad por nivel del Jefe de Policía
};
...
let userData = {
    ...
    // Ayuntamiento
    jefes_contratados: {},       // { piscina: true, fabrica: false, ... }
    jefe_policia_nivel: 0,
    // Comisaría
    policias: 0,
    tanques: 0,
    escudo_hasta: null
...
// RIPPLE
// ==========================================
function inyectarRipple() {
    const selector = 'button, .building-card, .nav-item, .action-banner, .centro-card, .city-edificio, .idioma-opcion, .genero-opcion, .theme-btn, .modal-back, .asistente-boton, .copy-btn, .upgrade-btn, .retiro-card, .sequence-card, .ranking-fila, .daily-day, .valeria-consejo, .boss-card, .raid-target, .ayuntamiento-card';
...
function getTotalProduction() {
    let base = 0;
    for (const id of ['piscina','fabrica','escuela','hospital']) {
        base += (userData['lvl_' + id] || 0) * ECONOMIA.PRODUCCION[id];
    }
...
function renderizarFeedNoticias() {
    ...
    if (evs.length === 0) {
        c.innerHTML = '<div class="feed-item"><div class="feed-icono">👋</div><div class="feed-texto"><div class="feed-titulo">Bienvenido a su ciudad</div><div class="feed-subtitulo">Los eventos aparecerán aquí</div></div></div>';
        return;
    }
    let h = '';
...
// AYUNTAMIENTO
// ==========================================
function openAyuntamiento() {
    closeAll();
    renderJefes();
    showModal('modalAyuntamiento');
    setActiveNav('ciudad');
}

function getJefeCosto(edificioId) {
    const prodBase = ECONOMIA.PRODUCCION[edificioId] * (1 + (userData['lvl_' + edificioId] || 0));
    return Math.floor(prodBase * ECONOMIA.JEFE_COSTO_MULT);
}

function renderJefes() {
    const cont = document.getElementById('lista-jefes'); if (!cont) return;
    const edificios = [
        { id: 'piscina', ico: '🏊', nombre: 'Piscina' },
        { id: 'fabrica', ico: '🏭', nombre: 'Fábrica' },
        { id: 'escuela', ico: '🏫', nombre: 'Escuela' },
        { id: 'hospital',ico: '🏥', nombre: 'Hospital' }
    ];
    let h = '';
    for (const e of edificios) {
        const contratado = !!userData.jefes_contratados[e.id];
        const costo = getJefeCosto(e.id);
        const prodBase = ECONOMIA.PRODUCCION[e.id] * (userData['lvl_' + e.id] || 0);
        h += '<div class="boss-card">';
        h += '<div class="boss-icon">' + e.ico + '</div>';
        h += '<div class="boss-info">';
        h += '<div class="boss-nombre">Jefe de ' + e.nombre + '</div>';
        h += '<div class="boss-detalle">' + (contratado
            ? '✅ Activo · cobra ' + Math.round(ECONOMIA.JEFE_SALARIO_PCT * 100) + '% (recogida automática)'
            : '💤 Sin jefe · buffer 12h · ' + costo.toLocaleString() + ' 💎') + '</div>';
        h += '</div>';
        h += '<button class="boss-btn" onclick="toggleJefe(\'' + e.id + '\')">' + (contratado ? 'DESPEDIR' : 'CONTRATAR') + '</button>';
        h += '</div>';
    }

    // Jefe de policía
    const costoJefeP = Math.floor(500 * Math.pow(2, userData.jefe_policia_nivel || 0));
    h += '<div class="boss-card" style="border-color:rgba(59,130,246,0.4);">';
    h += '<div class="boss-icon" style="background:rgba(59,130,246,0.15);">👮</div>';
    h += '<div class="boss-info">';
    h += '<div class="boss-nombre">Jefe de Policía (Nvl ' + (userData.jefe_policia_nivel || 0) + ')</div>';
    h += '<div class="boss-detalle">+ ' + Math.round((userData.jefe_policia_nivel || 0) * ECONOMIA.JEFE_POLICIA_BONUS * 100) + '% efectividad vs tanques · ' + costoJefeP.toLocaleString() + ' 💎</div>';
    h += '</div>';
    h += '<button class="boss-btn" onclick="mejorarJefePolicia()">MEJORAR</button>';
    h += '</div>';

    cont.innerHTML = h;
}

async function toggleJefe(edificioId) {
    const contratado = !!userData.jefes_contratados[edificioId];
    if (contratado) {
        if (!confirm('¿Despedir al jefe de ' + edificioId + '? Volverás a tener buffer de 12h.')) return;
        userData.jefes_contratados[edificioId] = false;
        registrarEvento('👔', 'Jefe despedido', edificioId);
    } else {
        const costo = getJefeCosto(edificioId);
        if (userData.diamonds < costo) return alert('❌ Necesitas ' + costo + ' 💎');
        if (!confirm('¿Contratar jefe por ' + costo.toLocaleString() + ' 💎?\n\nCon jefe, el edificio se recauda automáticamente cada hora y el jefe retiene el 15%.')) return;
        userData.diamonds -= costo;
        userData.jefes_contratados[edificioId] = true;
        registrarEvento('👔', 'Jefe contratado', edificioId + ' · ' + costo + ' 💎');
    }
    await saveUserData();
    actualizarUI();
    renderJefes();
...
async function mejorarJefePolicia() {
    const nivel = userData.jefe_policia_nivel || 0;
    const costo = Math.floor(500 * Math.pow(2, nivel));
    if (userData.diamonds < costo) return alert('❌ Necesitas ' + costo + ' 💎');
    if (!confirm('¿Subir Jefe de Policía a nivel ' + (nivel + 1) + ' por ' + costo.toLocaleString() + ' 💎?')) return;
    userData.diamonds -= costo;
    userData.jefe_policia_nivel = nivel + 1;
    registrarEvento('👮', 'Jefe de Policía mejorado', 'Nivel ' + (nivel + 1));
    await saveUserData();
    actualizarUI();
    renderJefes();
}
...
// COMISARÍA
// ==========================================
function openPolicia() {
    closeAll();
    renderPolicia();
    renderObjetivos();
    showModal('modalPolicia');
    setActiveNav('ciudad');
}

function renderPolicia() {
    const c = document.getElementById('policia-count');
    if (c) c.textContent = userData.policias || 0;
    const co = document.getElementById('policia-costo');
    const costo = Math.floor(ECONOMIA.POLICIA_COSTO_BASE * Math.pow(ECONOMIA.POLICIA_CRECIMIENTO, userData.policias || 0));
    if (co) co.textContent = costo.toLocaleString() + ' 💎';
    const b = document.getElementById('policia-bonus');
    if (b) b.textContent = '+' + Math.round((userData.jefe_policia_nivel || 0) * ECONOMIA.JEFE_POLICIA_BONUS * 100) + '%';
    const t = document.getElementById('policia-tanques');
    if (t) t.textContent = userData.tanques || 0;
    const btn = document.getElementById('policia-btn');
    if (btn) {
        if (userData.diamonds < costo) { btn.disabled = true; btn.textContent = '💎 INSUFICIENTE'; }
        else { btn.disabled = false; btn.textContent = 'CONTRATAR POLICÍA (' + costo.toLocaleString() + ' 💎)'; }
    }
}

async function contratarPolicia() {
    const costo = Math.floor(ECONOMIA.POLICIA_COSTO_BASE * Math.pow(ECONOMIA.POLICIA_CRECIMIENTO, userData.policias || 0));
    if (userData.diamonds < costo) return alert('❌ Insuficiente');
    userData.diamonds -= costo;
    userData.policias = (userData.policias || 0) + 1;
    registrarEvento('🚔', 'Policía contratado', '+' + costo + ' 💎');
    await saveUserData();
    actualizarUI();
    renderPolicia();
}
...
function renderObjetivos() {
    const c = document.getElementById('lista-objetivos'); if (!c) return;
    if (!CONFIG.PVP_ENABLED) {
        c.innerHTML = '<div class="game-instructions"><i class="fa-solid fa-lock"></i> Los raids estarán disponibles cuando el Worker seguro esté activo. Mientras tanto, puedes ver la lista teórica de objetivos.</div>';
    }
    // Lista simulada (solo lectura) de objetivos disponibles por rango
    const top = globalPoolData.user_rankings.slice(0, 10);
    if (top.length === 0) { c.innerHTML += '<div style="text-align:center;color:var(--text-secondary);padding:14px;">Sin objetivos por ahora.</div>'; return; }
    let h = '';
    for (const j of top) {
        if (j.id === userData.id) continue;
        const diff = Math.floor(j.produccion);
        if (diff > getTotalProduction() * 3) continue; // no listar los muy superiores
        h += '<div class="raid-target">';
        h += '<div><div style="font-weight:700;">🏙️ ' + j.city_name + '</div><div style="font-size:11px;color:var(--text-secondary);">Alcalde ' + (j.first_name || j.username) + ' · ⚡' + diff + '/h</div></div>';
        h += '<button style="background:#ef4444;border:none;color:white;padding:8px 14px;border-radius:12px;font-weight:800;font-size:11px;opacity:0.5;" disabled>PRONTO</button>';
        h += '</div>';
    }
    c.innerHTML += h;
}
...
function actualizarUI() {
    ...
    ['piscina','fabrica','escuela','hospital','policia'].forEach(id => {
        const el = document.getElementById('lvl_' + id);
        if (el) el.textContent = id === 'policia' ? (userData.policias || 0) : userData['lvl_' + id];
    });
    ...
    // Ayuntamiento: cantidad de jefes activos
    const badge = document.getElementById('ayuntamiento-badge');
    if (badge) {
        const n = Object.keys(userData.jefes_contratados || {}).filter(k => userData.jefes_contratados[k]).length;
        badge.textContent = n + ' jefe' + (n === 1 ? '' : 's');
    }
    const sub = document.getElementById('ayuntamiento-sub');
    if (sub) {
        const tieneJefes = Object.keys(userData.jefes_contratados || {}).some(k => userData.jefes_contratados[k]);
        sub.textContent = tieneJefes ? 'Recogida automática cada hora' : 'Contrata jefes para automatizar la producción';
    }
}
...
function closeAll() {
    ...
    ['modalPerfil','modalFriends','modalCiudad','modalRanking','modalBank','modalStore',
     ...
     'modalAyuntamiento','modalPolicia'
...
function actualizarRankingModal() {
    ...
    top.forEach((j, i) => {
        ...
        h += '<div class="ranking-fila' + (esYo ? ' yo' : '') + '"><div class="ranking-pos">' + medalla + '</div><div class="ranking-info"><div class="ranking-ciudad">🏙️ ' + j.city_name + '</div><div class="ranking-alcalde">' + (esYo ? 'Tú · ' : '') + titulo + ' ' + nombre + '</div></div><div class="ranking-produccion">⚡' + Math.floor(j.produccion) + '/h</div></div>';
    });
    l.innerHTML = h;
}
...
// ==========================================
function openBank() {
    closeAll(); showModal('modalBank'); cambiarMetodoPago('gram'); setActiveNav('ciudad');
}
...
// ==========================================
function openStore() {
    closeAll(); showModal('modalStore'); cambiarMetodoPagoPremium('gram'); setActiveNav('ciudad');
}
...
async function initAds() {
    try {
        ...
        if (typeof TelegramAdsController !== 'undefined') {
            ...
            richAdsReady = true;
            console.log('✅ RichAds inicializado');
        } else {
            richAdsReady = false;
            console.warn('⚠️ RichAds no cargó');
        }
    } catch (e) { richAdsReady = false; console.error(e); }
}

function showAdsModal() {
    closeAll(); showModal('modalAds'); actualizarEstadoAnuncio(); setActiveNav('ciudad');
...
function actualizarEstadoAnuncio() {
    ...
    if (!richAdsReady || typeof TelegramAdsController === 'undefined') {
        b.disabled = true;
        if (e) e.innerHTML = '⚠️ Anuncios no disponibles.';
...
async function reintentarRichAds() {
    ...
    } catch (err) { richAdsReady = false; }
    setTimeout(actualizarEstadoAnuncio, 800);
}

function showRichAds() {
    if (esPremium()) {
        alert('⭐ Eres Premium: sin anuncios.');
        return;
    }
    if (!richAdsReady || !window.TelegramAdsController) {
        reintentarRichAds();
        alert('📺 La red de anuncios se está recargando.');
        return;
    }
    try {
        window.TelegramAdsController.triggerNativeNotification(true)
            ...
            .catch(() => { alert('❌ No se pudo mostrar el anuncio.'); actualizarEstadoAnuncio(); });
    } catch (e) { alert('❌ Error'); }
}
...
function rescueWithAd() {
    ...
    if (!richAdsReady || !window.TelegramAdsController) { reintentarRichAds(); return alert('📺 Red recargando.'); }
    window.TelegramAdsController.triggerNativeNotification(true)
        .then(darRescate)
...
// ==========================================
// CENTRO FINANCIERO (abreviado por espacio, igual al v3.4)
// ==========================================
function openCasino() {
    closeAll();
...
function prepararTimingUI() {
    tZI = 30 + Math.random() * 30; tZA = 8 + Math.random() * 12;
...
function renderMatchGrid(act) {
    ...
    let h = '';
    for (let i = 0; i < 9; i++) h += '<div class="sequence-card" data-idx="' + i + '" onclick="clickMatch(' + i + ')" style="cursor:' + (act ? 'pointer' : 'default') + ';">' + SIMBOLOS_MATCH[i] + '</div>';
    c.innerHTML = h;
}
...
async function siguienteRondaMatch() { for (let i = 0; i < SIMBOLOS_POR_RONDA; i++) mSec.push(Math.floor(Math.random() * 9)); await mostrarSecuencia(); }
...
function clickMatch(idx) {
    ...
    if (mIn[pos] !== mSec[pos]) {
        mMos = true;
        const r = document.getElementById('match-result');
        if (r) r.innerHTML = '<span style="color:#ef4444;">❌ Fallaste en la ronda ' + mRon + '</span>';
    ...
    if (mIn.length === mSec.length) {
        ...
        else { mMos = true; setTimeout(siguienteRondaMatch, 1200); }
    }
}
...
function finalizarMatch() {
    ...
    const r = document.getElementById('match-result');
    if (r) r.innerHTML = '<span style="color:#4ade80;font-size:20px;">🧠 ¡MAESTRO! +' + pre + ' 💎</span>';
    spawnConfetti(); destelloResultado('modalMatch', true);
    actualizarUI(); saveUserData();
    const b = document.getElementById('match-balance'); if (b) b.textContent = Math.floor(userData.diamonds);
...
function renderPortfolio() {
    ...
    let h = '', tot = 0;
    for (const k of cl) { const a = ACCIONES_BOLSA.find(x => x.id === k); if (!a) continue; const v = p[a.id] * a.precio; tot += v; h += '<div style="display:flex;justify-content:space-between;padding:4px 0;"><span>' + a.icono + ' ' + p[a.id] + ' × ' + a.nombre + '</span><span>' + v + ' 💎</span></div>'; }
...
function comprarAccion(id) { const a = ACCIONES_BOLSA.find(x => x.id === id); if (!a) return; if (userData.diamonds < a.precio) return alert('❌'); userData.diamonds -= a.precio; if (!userData.bolsa_portfolio) userData.bolsa_portfolio = {}; userData.bolsa_portfolio[id] = (userData.bolsa_portfolio[id] || 0) + 1; registrarJugada('bolsa'); actualizarUI(); saveUserData(); renderBolsa(); }
function venderAccion(id) { const a = ACCIONES_BOLSA.find(x => x.id === id); if (!a) return; if (!userData.bolsa_portfolio || !userData.bolsa_portfolio[id]) return alert('❌'); userData.bolsa_portfolio[id]--; userData.diamonds += a.precio; registrarJugada('bolsa'); actualizarUI(); saveUserData(); renderBolsa(); }
function tickBolsa() { for (const a of ACCIONES_BOLSA) { const cp = (Math.random() - 0.48) * 8; a.precio = Math.max(10, Math.floor(a.precio * (1 + cp / 100))); a.tendencia = cp; } const m = document.getElementById('modalBolsa'); if (m && m.style.display === 'block') renderBolsa(); }
...
function renderSubasta() {
    ...
    let h = '<div style="background:rgba(250,204,21,0.08);border-left:4px solid var(--primary);padding:12px 14px;border-radius:12px;margin-bottom:14px;"><div style="font-weight:800;color:var(--primary);font-size:13px;">🧪 Modo Práctica</div><div style="font-size:11px;color:var(--text-secondary);margin-top:4px;">Las subastas aún no son P2P real.</div></div>';
    const ah = Date.now();
    for (const it of subastaItems) {
        const r = Math.max(0, Math.floor((it.termina - ah) / 1000));
        const m = Math.floor(r / 60), s = r % 60;
        const act = r > 0;
        h += '<div style="background:var(--bg-elevated);border-radius:16px;padding:14px;margin-bottom:10px;border:1px solid var(--glass-border);"><div style="display:flex;justify-content:space-between;align-items:center;"><div><div style="font-weight:700;">' + it.icono + ' ' + it.nombre + '</div><div style="font-size:12px;color:var(--text-secondary);">' + it.precioActual + ' 💎 · ' + m + ':' + String(s).padStart(2, '0') + '</div>';
...
function renderExpedicion() {
    ...
    for (const z of ZONAS_EXPEDICION) {
        h += '<div style="background:var(--bg-elevated);border-radius:16px;padding:14px;margin-bottom:10px;border:1px solid var(--glass-border);"><div style="font-weight:700;margin-bottom:6px;">' + z.icono + ' ' + z.nombre + '</div><div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">' + z.costo + ' 💎 · ' + z.duracion + 's · Riesgo ' + Math.floor(z.riesgo * 100) + '% · Retorno ' + z.recompensaBase + ' 💎</div><button onclick="lanzarExpedicion(\'' + z.id + '\')" style="background:#f97316;border:none;color:white;padding:10px 20px;border-radius:12px;font-weight:700;width:100%;">LANZAR</button></div>';
    }
...
function renderCrafting() {
    ...
    for (const r of RECETAS_CRAFT) {
        ...
        h += '<div style="background:var(--bg-elevated);border-radius:16px;padding:14px;margin-bottom:10px;border:1px solid var(--glass-border);"><div style="font-weight:700;">' + r.icono + ' ' + r.nombre + ' (Nvl ' + n + ')</div><div style="font-size:12px;color:var(--text-secondary);margin:6px 0;">Próximo: Nvl ' + (n + 1) + ' · ' + co + ' 💎</div><button onclick="craftear(\'' + r.id + '\')" style="background:' + (pu ? '#f472b6' : '#334155') + ';color:' + (pu ? '#000' : '#fff') + ';border:none;padding:10px 20px;border-radius:12px;font-weight:700;width:100%;" ' + (pu ? '' : 'disabled') + '>CRAFTEAR</button></div>';
    }
    c.innerHTML = h;
...
setInterval(() => { tickBolsa(); const me = document.getElementById('modalExpedicion'); if (me && me.style.display === 'block') renderExpedicionesActivas(); const ms = document.getElementById('modalSubasta'); if (ms && ms.style.display === 'block') renderSubasta(); }, 30000);
setInterval(() => { const me = document.getElementById('modalExpedicion'); if (me && me.style.display === 'block') renderExpedicionesActivas(); }, 1000);
    ...
    try {
        ...
        if (!r.error && r.data) {
            ...
            globalPoolData.user_rankings = r.data.map(u => {
                let p = (u.lvl_escuela || 0) * ECONOMIA.PRODUCCION.escuela + (u.lvl_fabrica || 0) * ECONOMIA.PRODUCCION.fabrica + (u.lvl_piscina || 0) * ECONOMIA.PRODUCCION.piscina + (u.lvl_hospital || 0) * ECONOMIA.PRODUCCION.hospital;
                if (u.premium_expires && new Date(u.premium_expires) > ah) p *= ECONOMIA.PREMIUM_MULTIPLICADOR;
                return { id: u.telegram_id, first_name: u.first_name || '', username: u.username || 'Alcalde', genero: u.genero || 'M', city_name: u.city_name || 'Ciudad sin nombre', diamonds: Number(u.diamonds) || 0, produccion: p };
            }).sort((a, b) => b.produccion - a.produccion);
        }
        const pos = globalPoolData.user_rankings.findIndex(u => u.id === userData.id);
        if (pos !== -1) {
            if (pos < 3) userData.rank = 'Diamante'; else if (pos < 10) userData.rank = 'Oro'; else if (pos < 50) userData.rank = 'Plata'; else userData.rank = 'Ciudadano';
        ...
        else { const ci = globalPoolData.user_rankings.slice(50); let t = 0; for (const c of ci) t += c.produccion; userData.projectedReward = (t > 0 && getTotalProduction() > 0) ? (PS * 0.15) * (getTotalProduction() / t) : 0; }
    } catch (e) { console.error('Error ranking:', e); }
}

// ==========================================
// GUARDADO (CORREGIDO - sin diamonds)
// ==========================================
async function saveUserData() {
    ...
    try {
        const d = {
            ...
            // NO escribimos diamonds, diamonds_soft, diamonds_hard ni premium_expires.
            // Solo el Worker con service_role puede tocarlos.
            ...
            jefes_contratados: userData.jefes_contratados || {},
            jefe_policia_nivel: userData.jefe_policia_nivel || 0,
            policias: userData.policias || 0,
            tanques: userData.tanques || 0
        };
        // SOLO si diamonds está permitido en RLS temporalmente
        // d.diamonds = Math.floor(userData.diamonds);
    ...
    try {
        ...
        if (!r.data) {
            ...
            userData = Object.assign({}, userData, d, {
                ...
                jefes_contratados: d.jefes_contratados || {},
                jefe_policia_nivel: d.jefe_policia_nivel || 0,
                policias: Number(d.policias) || 0,
                tanques: Number(d.tanques) || 0
            });
...
function construirConsejosValeria() {
    ...
    if (uR !== hoy) consejos.push({ icono: '🎁', texto: nombre + ', la recaudación diaria de ' + ciudad + ' está lista.', accion: 'daily', cta: 'Reclamar ahora' });
    if (tesoreria < 200 && !esPremium()) consejos.push({ icono: '📺', texto: 'Las arcas están bajas (' + tesoreria + ' 💎). Un anuncio le daría ' + ECONOMIA.ANUNCIO_RECOMPENSA + ' 💎 gratis.', accion: 'ads', cta: 'Ver anuncio' });
    ...
    for (const b of ['piscina','fabrica','escuela','hospital']) {
        ...
        if (!mejor || costo < mejor.costo) mejor = { b, n, costo };
    }
    if (mejor) {
        const nombres = { piscina:'la Piscina', fabrica:'la Fábrica', escuela:'la Escuela', hospital:'el Hospital' };
        if (tesoreria >= mejor.costo) consejos.push({ icono: '🏗️', texto: 'Puede mejorar ' + nombres[mejor.b] + ' al nivel ' + (mejor.n + 1) + ' por ' + mejor.costo.toLocaleString() + ' 💎.', accion: 'upgrade:' + mejor.b, cta: 'Mejorar ' + nombres[mejor.b] });
    }

    // Nuevo: consejo de Ayuntamiento
    const nJefes = Object.keys(userData.jefes_contratados || {}).filter(k => userData.jefes_contratados[k]).length;
    if (nJefes === 0 && tesoreria >= 1000) consejos.push({ icono: '🏛️', texto: 'Aún no ha contratado jefes. Con uno, la producción se recoge sola cada hora.', accion: 'ayuntamiento', cta: 'Ir al Ayuntamiento' });

    if (!esPremium()) consejos.push({ icono: '👑', texto: 'Con Premium duplica producción y elimina anuncios.', accion: 'premium', cta: 'Ver planes' });
    if ((userData.referred_users || []).length === 0) consejos.push({ icono: '👥', texto: 'Sin referidos. Cada amigo le paga el 10% de por vida.', accion: 'friends', cta: 'Invitar amigos' });

    if (consejos.length === 0) consejos.push({ icono: '✅', texto: 'Todo en orden en ' + ciudad + '.', accion: 'centro', cta: 'Ir al Centro Financiero' });
...
function valeriaAccion(accion) {
    ...
    switch (accion) {
        ...
        case 'ayuntamiento': openAyuntamiento(); break;
        case 'policia': openPolicia(); break;
    }
}
...
async function initApp() {
    console.log('🔄 Iniciando DIAMOND CITY v4.0...');
    tg.expand(); tg.ready();
    inyectarRipple();
    let usuario = null;
    if (tg.initDataUnsafe && tg.initDataUnsafe.user) usuario = tg.initDataUnsafe.user;
    ...
    actualizarUI();
    actualizarPremiumUI();
    console.log('✅ DIAMOND CITY v4.0 listo');
}
...
window.openAyuntamiento = openAyuntamiento;
window.openPolicia = openPolicia;
window.toggleJefe = toggleJefe;
window.mejorarJefePolicia = mejorarJefePolicia;
window.contratarPolicia = contratarPolicia;
