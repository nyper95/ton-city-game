// ======================================================
// DIAMOND CITY - v3.4 FINAL
// Economía centralizada · Navbar persistente · Ciudad = pantalla principal
// Valeria con hipervínculos · Retiros USDT TON (min 10) · RichAds
// ======================================================
console.log('🚀 DIAMOND CITY v3.4');
...
const CONFIG = {
    ...
    PRECIO_COMPRA: 0.001,          // 1 GRAM = 1,000 💎
    TON_A_STARS: 200,
    MARGEN_STARS: 1.15,
    ...
    API_BASE: ''                   // pon aquí la URL de tu Cloudflare Worker
};
...
function inyectarRipple() {
    const selector = 'button, .building-card, .nav-item, .action-banner, .centro-card, .city-edificio, .idioma-opcion, .genero-opcion, .theme-btn, .modal-back, .asistente-boton, .copy-btn, .upgrade-btn, .retiro-card, .sequence-card, .ranking-fila, .daily-day, .valeria-consejo';
...
// ==========================================
// ASISTENTE VALERIA (con hipervínculos)
// ==========================================
...
function getJugadasRestantes() {
    if (userData.haInvertido) return null;
...
// ==========================================
// CIUDAD → pantalla principal (cierra modal, marca navbar, hace scroll)
// ==========================================
function openCity() {
    closeAll();
    setActiveNav('ciudad');
...
async function initApp() {
    console.log('🔄 Iniciando DIAMOND CITY v3.4...');
    tg.expand(); tg.ready();
    inyectarRipple();
    let usuario = null;
    if (tg.initDataUnsafe && tg.initDataUnsafe.user) usuario = tg.initDataUnsafe.user;
    ...
    console.log('✅ DIAMOND CITY v3.4 listo');
}
